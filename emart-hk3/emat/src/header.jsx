...
function Header({ username }) {
    ...
    const handleLogout = () => {
        axios.get('http://localhost:3004/auth/logout')
            .then(() => {
                updateUserId(null); // Cập nhật userId thành null
                console.log('Cookie trước khi xóa:', req.cookies); // Kiểm tra cookie hiện tại
                res.clearCookie('token');
                console.log('Cookie sau khi xóa:', req.cookies); // Kiểm tra cookie sau khi xóa
                return res.json({ logout: true }); // Điều hướng về trang đăng nhập sau khi đăng xuất
            })
